...
impl Db {
    ...
    pub async fn get_public_rewards_by_streamer_id(&self, streamer_id: &str) -> DbResult<Vec<Reward>> {
        let rewards = sqlx::query_as::<_, Reward>(reward_select!("WHERE streamer_id = $1 AND NOT is_deleted AND is_public"))
            .bind(streamer_id)
            .fetch_all(&self.pool)
            .await?;
        Ok(rewards)
